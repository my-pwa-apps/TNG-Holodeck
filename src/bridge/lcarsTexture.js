...
export function drawLCARS(ctx, w, h, t = 0, title = 'SYS') {
  ...
  // Left bumpers
  [LC.orange, LC.blue, LC.purple, LC.darkOrange].forEach((c, i) => {
    rrect(ctx, 0, 4 + i * 44, 22, 36, 10, c);
  });

  // Top header bar
  rrect(ctx, 26, 0, w - 26, 36, [0, 10, 0, 0], LC.orange);
  ...
  systems.forEach((sys, i) => {
    ...
    rrect(ctx, 26, y, 90, 28, 4, sys.color);
    ctx.fillStyle = LC.bg;
...
export function createStaticLCARSTexture(variant = 0, size = 512) {
  ...
  const palette = [LC.orange, LC.blue, LC.purple, LC.teal,
                    LC.darkOrange, LC.peach, LC.lavender];
  const pick = i => palette[(i + variant) % palette.length];

  // Header bar
  rrect(ctx, 0, 0, size, h * 0.14, 8, pick(0));

  // Left bumper column
  for (let i = 0; i < 4; i++) {
    rrect(ctx, 0, h * 0.18 + i * h * 0.2, size * 0.06, h * 0.16, 6, pick(i + 1));
  }

  // Content blocks
  for (let row = 0; row < 4; row++) {
    const y  = h * 0.18 + row * h * 0.2;
    rrect(ctx, size * 0.08, y, size * 0.22, h * 0.16, 4, pick(row + 2));
    ctx.fillStyle = '#111122';
    ctx.fillRect(size * 0.32, y + h * 0.04, size * 0.64, h * 0.08);
    const fill = 0.4 + 0.5 * Math.abs(Math.sin(variant + row * 1.7));
    ctx.fillStyle = pick(row + 5);
    ctx.fillRect(size * 0.32, y + h * 0.04, size * 0.64 * fill, h * 0.08);
  }
